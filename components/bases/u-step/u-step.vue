    ...
    <div class="mt-2 space-y-1 flex flex-col justify-start items-stretch">
      <div v-for="(item, index) in items" :key="index + '__u-step-item'" @click="onSelectStepItem(item?.value)">
        <slot name="item" :item="item">
          <u-step-item v-bind="item" :collapse="collapse" :passed="currentStepIndex > index" :current="currentStepIndex == index" :last="items && index === (items?.length - 1)" :step="index + 1" :is-active="item?.value === currentStep"></u-step-item>
        </slot>
      </div>
    </div>
  </div>
