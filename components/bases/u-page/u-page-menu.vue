<template>
    <u-button v-if="!state" class="fixed bottom-2 left-2 md:bottom-2 md:left-16 z-50 rounded-full ring-4 ring-gray-50 border border-gray-500"
        size="sm" color="white" icon="i-heroicons-arrows-pointing-out" @click="onToggleMenu"></u-button>
        ...
        <u-vertical-navigation :links="links" :ui="navUI">
            <template #icon="{ link }">
                <u-avatar class="bg-gray-100 ring-2 ring-gray-50">
                    <u-icon :name="link.icon" class="w-4 h-4 flex-shrirk-0"></u-icon>
                </u-avatar>
            </template>
