    ...
    <u-form :state="state" class="flex flex-col justify-start items-stretch gap-y-8">
      <div class="flex flex-col gap-y-1">
        <u-avatar size="lg" class="bg-primary-100 border-8 border-primary-50">
          <u-icon size="xl" name="i-heroicons-shield-check-20-solid" class="text-primary-500" />
        </u-avatar>
        <h6 class="font-semibold text-xl leading-0">
          New member!
        </h6>
        <span class="text-sm text-gray-500 leading-0">Workspace colaborator with your team</span>
      </div>
      <div class="flex flex-col justify-start items-stretch gap-y-2">
        <u-form-group :label="$t('general.form.Workspace')">
          <u-input v-model="state.workspace" disabled color="gray" size="lg" />
        </u-form-group>
        <u-form-group :label="$t('general.form.Email')">
          <template #hint>
            <u-button to="/auth/login" variant="ghost" size="xs">
              Have account? Login
            </u-button>
          </template>
        ...
        <u-form-group :label="$t('general.form.Confirm password')">
          <u-input v-model="state.confirmPassword" size="lg" type="password" />
        </u-form-group>
      </div>
      <div class="flex flex-col justify-start items-stretch gap-y-2">
...
}>(authRegisterInjectionKey, { state: {} })
</script>